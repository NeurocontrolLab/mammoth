...
data_path_ = os.path.join(data_path,'sorted_data','mountainsort5_output')
...
for ind, border in tqdm(enumerate(range(0,1024,128))):
    sp = os.path.join(data_path_,"mountain_shank_{}".format(ind))
    ...
    try:
        if len(os.listdir(sp)) == 0:
            ...
            sorting = se.read_npz_sorting(
                os.path.join(sp, 'sorter_output/firings.npz'))
...
if wrong_flag == 0:
    with open(os.path.join(data_path_, 'automount.txt'), 'w') as file:
        file.write("")      
else:
